...
# File: tb.tcl
# Generated on: Wed Sep 10 17:19:10 2025
...
if {[is_project_open]} {
	if {[string compare $quartus(project) "tb"]} {
		puts "Project tb is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists tb]} {
		project_open -revision tb tb
	} else {
		project_new -revision tb tb
	}
...
if {$make_assignments} {
	...
	set_global_assignment -name PROJECT_CREATION_TIME_DATE "10:01:44  SEPTEMBER 10, 2025"
	set_global_assignment -name LAST_QUARTUS_VERSION "22.1std.2 Lite Edition"
	set_global_assignment -name VHDL_FILE hex7seg.vhd
	set_global_assignment -name VHDL_FILE dig1pack.vhd
	set_global_assignment -name VHDL_FILE dcd.vhd
	set_global_assignment -name VHDL_FILE tb.vhd
	set_global_assignment -name PARTITION_NETLIST_TYPE SOURCE -section_id Top
	set_global_assignment -name PARTITION_FITTER_PRESERVATION_LEVEL PLACEMENT_AND_ROUTING -section_id Top
	set_global_assignment -name PARTITION_COLOR 16764057 -section_id Top
	set_global_assignment -name VHDL_FILE dfa_fsm.vhd
